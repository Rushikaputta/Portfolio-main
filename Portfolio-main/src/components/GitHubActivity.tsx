    ...
    const fetchGitHubData = async () => {
      try {
        ...
        for (let year = startYear; year <= currentYear; year++) {
          try {
            const yearResponse = await fetch(`https://github-contributions-api.jogruber.de/v4/Rushikaputta?y=${year}`);
        ...
        const reposResponse = await fetch('https://api.github.com/users/Rushikaputta/repos?sort=updated&per_page=3');
        const reposData = await reposResponse.json();
        setRecentRepos(reposData);

        // Fetch recent commits from user's events
        const eventsResponse = await fetch('https://api.github.com/users/Rushikaputta/events?per_page=10');
